...
int BigInt:: operator < (const BigInt &bi) const //a<b
{
 ...
 BigInt operator_x_exponentiation_y_modN(BigInt x, BigInt y, BigInt N)
 {
	 ...
	 if (y%d == 0) return operator_a_multiply_b_modN(z,z,N);
	 else return operator_a_multiply_b_modN( x, operator_a_multiply_b_modN(z, z, N),N);
 }
 ...
 //}
	...
	// cout << "a^b=" << operator^(a, b);
	 ...
	 BigInt a("932"), b("234"), N("1000"), t("3");

	 //BigInt res = operator_a_multiply_b_modN(a,b,N);
	 BigInt  res = operator_x_exponentiation_y_modN(a,/*BigInt("3")*/ t,N); //
	 cout << "res=" << res<<endl;
